...
export class NetworkManager {
    ...
    setupRoomListeners() {
        ...
        // Listen for full game state snapshots from host (clients only)
        if (!this.isHost) {
    ...
    async sendICECandidate(candidate) {
        ...
        // Unwrap when called with { candidate: RTCIceCandidate }
        if (candidate && typeof candidate === 'object' && 'candidate' in candidate && candidate.candidate instanceof RTCIceCandidate) {
            candidateData = candidate.candidate;
        } else if (candidate && typeof candidate === 'object' && 'candidate' in candidate && typeof candidate.candidate === 'object' && candidate.candidate.candidate) {
            // Handle nested { candidate: { candidate: '...' } }
            candidateData = candidate.candidate;
        } else if (candidate && typeof candidate === 'object' && 'candidate' in candidate) {
            // RTCIceCandidate already
        ...
        if (candidateData.sdpMid !== undefined && candidateData.sdpMid !== null) {
            serializedCandidate.sdpMid = candidateData.sdpMid;
        }

        if (candidateData.sdpMLineIndex !== undefined && candidateData.sdpMLineIndex !== null) {
            serializedCandidate.sdpMLineIndex = candidateData.sdpMLineIndex;
        }

        if (candidateData.usernameFragment !== undefined && candidateData.usernameFragment !== null) {
            serializedCandidate.usernameFragment = candidateData.usernameFragment;
        }

        try {
            await this.db.ref(`rooms/${this.roomId}/webrtc/candidates`).push(serializedCandidate);
        } catch (error) {
